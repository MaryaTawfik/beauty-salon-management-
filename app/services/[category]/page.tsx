...
export default function CategoryPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  // 2. Pull dynamic data from Global Context
  const { categories, isMounted: servicesLoaded } = useServices();
  
  const [favorites, setFavorites] = useState<string[]>([]);
  const [isMounted, setIsMounted] = useState(false);
  
  // 3. Find the specific category from our dynamic list
              ...
              >
                <div className="flex flex-col md:flex-row items-center gap-8 w-full">
                  <div className="relative w-32 h-32 md:w-40 md:h-40 overflow-hidden border border-white/5">
                     <Image 
                      src={sub.images[0] || '/placeholder.jpg'} 
                      alt={sub.name} 
                      fill 
                  ...
                  <div className="flex-1 text-center md:text-left space-y-3">
                    ...
                    <p className="text-white/40 text-sm font-light line-clamp-2 max-w-xl">{sub.description}</p>
                ...
                <div className="mt-6 md:mt-0 flex items-center gap-4">
                  <button 
                    ...
                    className={cn(
                      ...
                      favorites.includes(sub.id) ? "bg-[#D4AF7A] text-black" : "border-white/10 text-white/40 hover:border-white"
                    )}
                  >
                    <Heart size={20} fill={favorites.includes(sub.id) ? "currentColor" : "none"} />
                  </button>
                  <ArrowRight size={20} className="text-[#D4AF7A] opacity-0 group-hover:opacity-100 transition-all" />
                </div>
              </Link>
            </motion.div>
          ))}
